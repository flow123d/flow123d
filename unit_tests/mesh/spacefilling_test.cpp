...
//     }
    ...
    void calculateHibert() {
        for (uint i = 0; i < points.size(); ++i) {
            points[i].hilbertIndex = hilbertIndex(points[i].coords[0], points[i].coords[1], sizes[points[i].originalIndex]);
        }
    }
    void sortByHilbert() {
...
//             indices[points[i].originalIndex] = points[i].hilbertIndex;
//             indices[points[i].originalIndex] = (groupIndex % 10);
//             indices[points[i].originalIndex] = groupIndex;
        }
    }
};
