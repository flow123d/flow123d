...
TEST(PVDReader, get_element_data) {
    ...
    for (unsigned int i=0; i<3; ++i) {
        ...
        typename ElementDataCache<double>::ComponentDataPtr scalar_data = reader.get_element_data<double>(6, 1, false, 0, 0.0);
    ...
    for (unsigned int i=0; i<3; ++i) {
    	...
    	reader.find_header(header_params);
        typename ElementDataCache<double>::ComponentDataPtr vector_data = reader.get_element_data<double>(6, 3, false, 0, 0.0);
