...
TEST_F(TestParallelOutput, continuous_mesh)
{
    ...
    if (rank == 0) {
        ...
        auto &node_vec = *( stream->nodes_cache()->get_component_data().get() );
        for (unsigned int i=0; i<node_vec.size(); ++i) {
            DebugOut() << "i node: " << i;
            EXPECT_DOUBLE_EQ(expected_nodes[i],  node_vec[i]);
        }

        auto &conn_vec = *( stream->connectivity_cache()->get_component_data().get() );
        for (unsigned int i=0; i<conn_vec.size(); ++i) {
            DebugOut() << "i conn: " << i;
            EXPECT_EQ(expected_connectivities[i],  conn_vec[i]);
        }

        auto &offs_vec = *( stream->offsets_cache()->get_component_data().get() );
        ...
        for (unsigned int i=0; i<elem_cache_map.size(); ++i) {
            ...
            auto &current_vec = *( current_cache->get_component_data().get() );
