...
TEST(ReaderCache, get_bulk_element_data) {
    ...
    for (i=0; i<3; ++i) {
        typename ElementDataCache<int>::ComponentDataPtr multifield_data =
        		ReaderCache::get_reader(file_name)->get_element_data<int>("vector_fixed", 0.0, 9, 1,
        		false, i);
    ...
    // read data to one vector for Field
    {
    	typename ElementDataCache<int>::ComponentDataPtr field_data =
    			ReaderCache::get_reader(file_name)->get_element_data<int>("vector_fixed", 1.0, 9, 3,
                false, 0);
...
TEST(ReaderCache, get_boundary_element_data) {
    ...
    for (i=0; i<3; ++i) {
        typename ElementDataCache<int>::ComponentDataPtr multifield_data =
        		ReaderCache::get_reader(file_name)->get_element_data<int>("vector_fixed", 0.0, 4, 1,
        		true, i);
    	std::vector<int> &vec = *( multifield_data.get() );
    ...
    {
    	typename ElementDataCache<int>::ComponentDataPtr field_data =
    			ReaderCache::get_reader(file_name)->get_element_data<int>("vector_fixed", 1.0, 4, 3,
                true, 0);
    	std::vector<int> &vec = *( field_data.get() );
...
TEST(ReaderCache, find_header) {
    ...
    data = ReaderCache::get_reader(file_name)->get_element_data<double>("vector_fixed", 0.0, n_elements, n_comp, false, 0);
    EXPECT_EQ(1.0, (*data)[0]);
    EXPECT_EQ(2.0, (*data)[1]);
    EXPECT_EQ(3.0, (*data)[2]);
    EXPECT_EQ(3.0, (*data)[3*8+2]);

    data = ReaderCache::get_reader(file_name)->get_element_data<double>("vector_fixed", 0.1, n_elements, n_comp, false, 0);
    EXPECT_EQ(1.0, (*data)[0]);
    EXPECT_EQ(2.0, (*data)[1]);
    EXPECT_EQ(3.0, (*data)[2]);
    EXPECT_EQ(3.0, (*data)[3*8+2]);

    data = ReaderCache::get_reader(file_name)->get_element_data<double>("vector_fixed", 0.9, n_elements, n_comp, false, 0);
    EXPECT_EQ(1.0, (*data)[0]);
    EXPECT_EQ(2.0, (*data)[1]);
    EXPECT_EQ(3.0, (*data)[2]);
    EXPECT_EQ(3.0, (*data)[3*8+2]);

    data = ReaderCache::get_reader(file_name)->get_element_data<double>("vector_fixed", 1.0, n_elements, n_comp, false, 0);
    EXPECT_EQ(2.0, (*data)[0]);
    EXPECT_EQ(3.0, (*data)[1]);
    EXPECT_EQ(4.0, (*data)[2]);
    EXPECT_EQ(4.0, (*data)[3*8+2]);

    data = ReaderCache::get_reader(file_name)->get_element_data<double>("vector_fixed", 1.1, n_elements, n_comp, false, 0);
    EXPECT_EQ(2.0, (*data)[0]);
    EXPECT_EQ(3.0, (*data)[1]);
    EXPECT_EQ(4.0, (*data)[2]);
    EXPECT_EQ(4.0, (*data)[3*8+2]);

    data = ReaderCache::get_reader(file_name)->get_element_data<double>("vector_fixed", 2.1, n_elements, n_comp, false, 0);
    EXPECT_EQ(2.0, (*data)[0]);
    EXPECT_EQ(3.0, (*data)[1]);
    EXPECT_EQ(4.0, (*data)[2]);
    EXPECT_EQ(4.0, (*data)[3*8+2]);

    data = ReaderCache::get_reader(file_name)->get_element_data<double>("vector_fixed", 200, n_elements, n_comp, false, 0);
    EXPECT_EQ(2.0, (*data)[0]);
    EXPECT_EQ(3.0, (*data)[1]);
    EXPECT_EQ(4.0, (*data)[2]);
    EXPECT_EQ(4.0, (*data)[3*8+2]);
