...
TEST(ReaderInstance, get_bulk_element_data) {
	Profiler::initialize();
	unsigned int i, j;
    ...
    auto reader = ReaderInstance::get_reader(file_name);
    ...
    for (i=0; i<3; ++i) {
        typename ElementDataCache<int>::ComponentDataPtr multifield_data =
        		ReaderInstance::get_reader(file_name)->get_element_data<int>("vector_fixed", 0.0, 9, 1,
        		false, i);
    ...
    // read data to one vector for Field
    {
    	typename ElementDataCache<int>::ComponentDataPtr field_data =
        		ReaderInstance::get_reader(file_name)->get_element_data<int>("vector_fixed", 1.0, 9, 3,
                false, 0);
...
TEST(ReaderInstance, get_boundary_element_data) {
	Profiler::initialize();
	unsigned int i, j;
    ...
    for (i=0; i<3; ++i) {
        typename ElementDataCache<int>::ComponentDataPtr multifield_data =
        		ReaderInstance::get_reader(file_name)->get_element_data<int>("vector_fixed", 0.0, 4, 1,
        		true, i);
    	std::vector<int> &vec = *( multifield_data.get() );
    ...
    {
    	typename ElementDataCache<int>::ComponentDataPtr field_data =
        		ReaderInstance::get_reader(file_name)->get_element_data<int>("vector_fixed", 1.0, 4, 3,
                true, 0);
    	std::vector<int> &vec = *( field_data.get() );
...
TEST(ReaderInstance, find_header) {
    Profiler::initialize();
    ...
    data = ReaderInstance::get_reader(file_name)->get_element_data<double>("vector_fixed", 0.0, n_elements, n_comp, false, 0);
    EXPECT_EQ(1.0, (*data)[0]);
    EXPECT_EQ(2.0, (*data)[1]);
    EXPECT_EQ(3.0, (*data)[2]);
    EXPECT_EQ(3.0, (*data)[3*8+2]);

    data = ReaderInstance::get_reader(file_name)->get_element_data<double>("vector_fixed", 0.1, n_elements, n_comp, false, 0);
    EXPECT_EQ(1.0, (*data)[0]);
    EXPECT_EQ(2.0, (*data)[1]);
    EXPECT_EQ(3.0, (*data)[2]);
    EXPECT_EQ(3.0, (*data)[3*8+2]);

    data = ReaderInstance::get_reader(file_name)->get_element_data<double>("vector_fixed", 0.9, n_elements, n_comp, false, 0);
    EXPECT_EQ(1.0, (*data)[0]);
    EXPECT_EQ(2.0, (*data)[1]);
    EXPECT_EQ(3.0, (*data)[2]);
    EXPECT_EQ(3.0, (*data)[3*8+2]);

    data = ReaderInstance::get_reader(file_name)->get_element_data<double>("vector_fixed", 1.0, n_elements, n_comp, false, 0);
    EXPECT_EQ(2.0, (*data)[0]);
    EXPECT_EQ(3.0, (*data)[1]);
    EXPECT_EQ(4.0, (*data)[2]);
    EXPECT_EQ(4.0, (*data)[3*8+2]);

    data = ReaderInstance::get_reader(file_name)->get_element_data<double>("vector_fixed", 1.1, n_elements, n_comp, false, 0);
    EXPECT_EQ(2.0, (*data)[0]);
    EXPECT_EQ(3.0, (*data)[1]);
    EXPECT_EQ(4.0, (*data)[2]);
    EXPECT_EQ(4.0, (*data)[3*8+2]);

    data = ReaderInstance::get_reader(file_name)->get_element_data<double>("vector_fixed", 2.1, n_elements, n_comp, false, 0);
    EXPECT_EQ(2.0, (*data)[0]);
    EXPECT_EQ(3.0, (*data)[1]);
    EXPECT_EQ(4.0, (*data)[2]);
    EXPECT_EQ(4.0, (*data)[3*8+2]);

    data = ReaderInstance::get_reader(file_name)->get_element_data<double>("vector_fixed", 200, n_elements, n_comp, false, 0);
    EXPECT_EQ(2.0, (*data)[0]);
    EXPECT_EQ(3.0, (*data)[1]);
    EXPECT_EQ(4.0, (*data)[2]);
    EXPECT_EQ(4.0, (*data)[3*8+2]);
...
TEST(ReaderInstance, get_reader) {
	Profiler::initialize();
...

	{
		...
		ReaderInstance::get_reader(mesh1);
		ReaderInstance::get_reader(mesh2);
	}

	{
	    Input::Record i_rec = get_input_record("{mesh_file=\"mesh/test_input.msh\"}");
	    Mesh * mesh = new Mesh(i_rec);
	    auto mesh_reader = ReaderInstance::get_reader( i_rec.val<FilePath>("mesh_file") );
...
TEST(ReaderInstance, repeat_call) {
    FilePath::set_io_dirs(".",UNIT_TESTS_SRC_DIR,"",".");
