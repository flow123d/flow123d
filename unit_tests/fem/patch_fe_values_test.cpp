...
TEST(FeValues, patch_test_basic) {
    ...
    std::vector< ElementAccessor<3> > element_vec;
    for (auto elm : mesh->elements_range()) {
    	element_vec.push_back(elm);
    }

    // Reinitializes FE data of all 2D elements in one step
    patch_fe_values.reinit(element_vec);
    ...
    {
        if (element_vec[i].dim() != 2) continue;
        fe_values.reinit(element_vec[i]);
