...
//::testing::FLAGS_gtest_death_test_style = "threadsafe";
    ...
    EXPECT_THROW( {sel1->add_value(green,"red");}, ExcXprintfMsg); //"already exists in Selection:"
    EXPECT_THROW( {sel2.add_value(green,"red");}, ExcXprintfMsg); //"already exists in Selection:"
    EXPECT_THROW( {sel1->add_value(blue,"blue1");}, ExcXprintfMsg); //"conflicts with value"
    EXPECT_THROW( {sel2.add_value(blue,"blue1");}, ExcXprintfMsg); //"conflicts with value"


    sel2.add_value(green,"green");
    sel2.close();
    EXPECT_THROW( {sel2.add_value(yellow,"y");}, ExcXprintfMsg); //"in finished Selection type:"
