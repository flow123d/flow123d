import sys
import xml.etree.ElementTree as ET

def create_junit_xml(status, log_file, xml_output):
    """
    Vytvoří JUnit XML report na základě statusu kompilace.
    Pokud kompilace selže, nastaví test jako failed a přidá výstup.
    Pokud uspěje, nastaví test jako passed a přidá výstup.
    """
    with open(log_file, "r", encoding="utf-8") as f:
        compile_output = f.read()

    testsuites = ET.Element("testsuites", time="0.0")
    testsuite = ET.Element("testsuite", name="CompilationTests", tests="1", failures="1" if status != 0 else "0")
    testcase = ET.SubElement(testsuite, "testcase", classname="Compilation", name="CompileTest")

    if status != 0:
        failure = ET.SubElement(testcase, "failure", message="Compilation failed")
        failure.text = compile_output
    else:
        system_out = ET.SubElement(testcase, "system-out")
        system_out.text = compile_output

    tree = ET.ElementTree(testsuite)
    with open(xml_output, "wb") as xml_file:
        tree.write(xml_file, encoding="utf-8", xml_declaration=True)

if __name__ == "__main__":
    status = int(sys.argv[1])
    log_file = sys.argv[2]
    xml_output = sys.argv[3]

    create_junit_xml(status, log_file, xml_output)
