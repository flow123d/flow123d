...
void Field<spacedim, Value>::cache_update(ElementCacheMap &cache_map) {
    ...
    for (reg_elm_it=update_cache_data.region_element_map_.begin(); reg_elm_it!=update_cache_data.region_element_map_.end(); ++reg_elm_it) {
        std::unordered_map<unsigned int, unsigned int>::iterator bgn_it = update_cache_data.region_cache_indices_map_.find(reg_elm_it->first);
        unsigned int region_in_cache = update_cache_data.region_cache_indices_map_.find(reg_elm_it->first)->second;
        ...
        region_fields_[reg_elm_it->first]->cache_update(value_cache_, i_cache_el_begin, i_cache_el_end, reg_elm_it->second);
    }
}
