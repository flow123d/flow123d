...
void RichardsLMH::initialize_asm() {
    ...
    this->init_cond_postprocess_assembly_ = new GenericAssembly< InitCondPostprocessAssemblyDim >(this->eq_data_.get());
    this->mh_matrix_assembly_ = new GenericAssembly< MHMatrixAssemblyRichardsDim >(this->eq_data_.get());
    this->reconstruct_schur_assembly_ = new GenericAssembly< ReconstructSchurAssemblyRichardsDim >(this->eq_data_.get());
}
