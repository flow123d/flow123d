...
void RichardsLMH::initialize_asm() {
    ...
    this->mh_matrix_assembly_ = new GenericAssembly< MHMatrixAssemblyRichardsDim >(this->eq_fields_.get(), this->eq_data_.get());
    this->reconstruct_schur_assembly_ = new GenericAssembly< ReconstructSchurAssemblyRichardsDim >(this->eq_fields_.get(), this->eq_data_.get());
}
