...
void DarcyFlowMHOutput::prepare_output(Input::Record in_rec)
{
	...
	ele_pressure_ptr->set_fe_data(dh_scalar->sequential(), 0, VectorMPI::sequential(ele_pressure.size()) );
	...
	ele_piezo_head_ptr->set_fe_data(dh_scalar->sequential(), 0, VectorMPI::sequential(ele_piezo_head.size()) );
	...
	shared_ptr<DOFHandlerMultiDim> dh_vector = make_shared<DOFHandlerMultiDim>(*mesh_);
	{
		...
		FiniteElement<0> *felm0 = new FESystem<0>(fe0_ptr, FEType::FEVector, 3);
		FiniteElement<1> *felm1 = new FESystem<1>(fe1_ptr, FEType::FEVector, 3);
	...
	ele_flux_ptr->set_fe_data(dh_vector->sequential(), 0, VectorMPI::sequential(ele_flux.size()) );
...
void DarcyFlowMHOutput::prepare_specific_output(Input::Record in_rec)
{
	...
	diff_data.vel_diff_ptr->set_fe_data(dh_seq, 0, VectorMPI::sequential(diff_data.velocity_diff.size()) );
	...
	diff_data.pressure_diff_ptr->set_fe_data(dh_seq, 0, VectorMPI::sequential(diff_data.pressure_diff.size()) );
	...
	diff_data.div_diff_ptr->set_fe_data(dh_seq, 0, VectorMPI::sequential(diff_data.div_diff.size()) );
