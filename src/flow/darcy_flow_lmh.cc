...
void DarcyLMH::initialize_asm() {
    ...
    this->mh_matrix_assembly_ = new GenericAssembly< MHMatrixAssemblyLMHDim >(eq_fields_.get(), eq_data_.get());
    this->reconstruct_schur_assembly_ = new GenericAssembly< ReconstructSchurAssemblyLMHDim >(eq_fields_.get(), eq_data_.get());
}
