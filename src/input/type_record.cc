...
bool Default::check_validity(std::shared_ptr<TypeBase> type) const
{
	...
	try {
		...
		THROW( ExcWrongDefaultJSON() << EI_DefaultStr( value_ ) << EI_TypeName(type->type_name()) << EI_Nested(&e) );
	} catch ( Input::ReaderToStorage::ExcInputError &e ) {
		THROW( ExcWrongDefault() << EI_DefaultStr( value_ ) << EI_TypeName(type->type_name()) << EI_Nested(&e) );
	}
}
