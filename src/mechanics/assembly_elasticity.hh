...
public:
    ...
    inline void dimjoin_intergral(DHCellAccessor cell_lower_dim, DHCellSide neighb_side) {
        ...
        auto p_low = p_high.lower_dim(cell_lower_dim);
        // int side_sign = (neighb_side.side() == *neighb_side.side().edge().side(0)) ? 1 : -1;
        ...
        dof_indices_scalar_ = cell_scalar.get_loc_dof_indices();
        dof_indices_tensor_ = cell_tensor.get_loc_dof_indices();
        dof_indices_vector_ = cell_vector.get_loc_dof_indices();
        for (unsigned int i=0; i<3; i++)
            for (unsigned int j=0; j<3; j++)
...
private:
    ...
    double normal_displacement_;                              ///< Holds contributions of normal displacement.
    arma::mat33 normal_stress_;                               ///< Holds contributions of normal stress.
