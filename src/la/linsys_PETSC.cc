...
void LinSys_PETSC::start_add_assembly()
{
    switch ( status_ ) {
        ...
        case DONE:
            break;
        default:
        	ASSERT(false).error("Can not set values. Matrix is not preallocated.\n");
    }
    status_ = ADD;
}
...
void LinSys_PETSC::rhs_set_values( int nrow, int *rows, double *vals )
{
    ...
    switch (status_) {
        ...
        case ALLOCATE: 
            break;
        default: ASSERT(false).error("LinSys's status disallow set values.\n");
    }
...
{
	ASSERT_EQ_DBG(status_, ALLOCATE).error("Linear system has to be in ALLOCATE status.");
...
void LinSys_PETSC::apply_constrains( double scalar )
{
    ...
    ASSERT_EQ_DBG(status_, DONE).error("System matrix and right-hand side are not assembled when applying constraints." );
