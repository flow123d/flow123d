...
LinSys_PERMON::LinSys_PERMON( LinSys_PERMON &other )
	: LinSys_PETSC(other), params_(other.params_), v_rhs_(NULL), solution_precision_(other.solution_precision_)
{
...
//void LinSys_PERMON::set_tolerances(double  r_tol, double a_tol, unsigned int max_it)
//{
//    if (! in_rec_.is_empty()) {
//        // input record is set
...
void LinSys_PERMON::mat_set_values( int nrow, int *rows, int ncol, int *cols, double *vals )
{
...
void LinSys_PERMON::preallocate_values(int nrow,int *rows,int ncol,int *cols)
{
    int i,j;
    int col;
    PetscInt row;
...
void LinSys_PERMON::apply_constrains( double scalar )
{
    PetscErrorCode ierr;
...
void LinSys_PERMON::set_initial_guess_nonzero(bool set_nonzero)
{
