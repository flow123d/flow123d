...
public:
    ...
    inline void update_values(FV &fe_values, FMT_UNUSED const ElementValues<spacedim> &elm_values,
        ...
        for (unsigned int i = 0; i < fe_data.n_points; i++)
            for (unsigned int j = 0; j < fe_data.n_dofs; j++) {
            	fe_values.set_shape_value(i, j, fe_data.ref_shape_values[i][j][0] );
            }
    }
    ...
    inline void update_gradients(FV &fe_values, const ElementValues<spacedim> &elm_values,
        ...
        for (unsigned int i = 0; i < fe_data.n_points; i++)
            for (unsigned int j = 0; j < fe_data.n_dofs; j++)
                fe_values.set_shape_gradient(i, j, trans(elm_values.inverse_jacobian(i)) * fe_data.ref_shape_grads[i][j] );
    }
};
        ...
        for (unsigned int i = 0; i < fe_data.n_points; i++)
            for (unsigned int j = 0; j < fe_data.n_dofs; j++)
            {
                ...
                for (unsigned int c=0; c<spacedim; c++)
                    fe_values.set_shape_value(i, j*spacedim+c, fv_vec(c));
            }
    }
    ...
    inline void update_gradients(FV &fe_values, const ElementValues<spacedim> &elm_values,
        ...
        for (unsigned int i = 0; i < fe_data.n_points; i++)
            for (unsigned int j = 0; j < fe_data.n_dofs; j++)
            {
                ...
                for (unsigned int c=0; c<spacedim; c++)
                    fe_values.set_shape_gradient(i, j*spacedim+c, grads.col(c));
            }
    }
};
        ...
        for (unsigned int i = 0; i < fe_data.n_points; i++)
            for (unsigned int j = 0; j < fe_data.n_dofs; j++)
            {
                ...
                for (unsigned int c=0; c<spacedim*spacedim; c++)
                    fe_values.set_shape_value(i, j*spacedim*spacedim+c, fv_vec[c]);
            }
    }
    ...
    inline void update_gradients(FV &fe_values, const ElementValues<spacedim> &elm_values,
        ...
        for (unsigned int i = 0; i < fe_data.n_points; i++)
            for (unsigned int j = 0; j < fe_data.n_dofs; j++)
            {
                arma::mat grads = trans(elm_values.inverse_jacobian(i)) * fe_data.ref_shape_grads[i][j];
                for (unsigned int c=0; c<spacedim*spacedim; c++)
                    fe_values.set_shape_gradient(i, j*spacedim*spacedim+c, grads.col(c));
            }
    }
};
            ...
            for (unsigned int i=0; i<fe_data.n_points; i++)
                for (unsigned int n=0; n<fe_values.fe_sys_dofs_[f].size(); n++)
                    for (unsigned int c=0; c<fe_values.fe_sys_n_space_components_[f]; c++)
                        fe_values.set_shape_value( i, shape_offset+fe_values.n_components_*n+comp_offset+c,
                                fe_values.fe_values_vec[f].shape_value(i, n*fe_values.fe_sys_n_space_components_[f]+c) );
    ...
    inline void update_gradients(FV &fe_values, FMT_UNUSED const ElementValues<spacedim> &elm_values,
        ...
        for (unsigned int f=0; f<fe_values.fe_sys_dofs_.size(); f++)
        {
            ...
            for (unsigned int i=0; i<fe_data.n_points; i++)
                for (unsigned int n=0; n<fe_values.fe_sys_dofs_[f].size(); n++)
                    for (unsigned int c=0; c<fe_values.fe_sys_n_space_components_[f]; c++)
                        fe_values.set_shape_gradient(
                                i,
						        shape_offset+fe_values.n_components_*n+comp_offset+c,
                                fe_values.fe_values_vec[f].shape_grad(i, n*fe_values.fe_sys_n_space_components_[f]+c) );
