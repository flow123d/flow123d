...
class CoverageMerge (object):
    ...
    def execute_merge (self):
        ...
        if not self.xmlfiles:
            for filename in os.listdir (self.path):
                ...
                if fullname == self.finalxml: continue
        ...
        if self.filteronly:
            ...
            currfile = 1
            logging.debug ('{2}/{3} merging: {0} & {1}'.format (self.xmlfiles[0], self.xmlfiles[1], currfile, totalfiles-1))
    ...
    def filter_xml (self, xmlfile):
        ...
        included = []
        if self.packagefilters: logging.debug ('excluding packages:')
