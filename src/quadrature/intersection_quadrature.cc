...
/*
#include "intersection_quadrature.hh"
...
IntersectionQuadratureP0::IntersectionQuadratureP0( Mesh &mesh)

IntersectionQuadratureP0::reinit(const IntersectionLocalBase *isec)
:	dim(isec->n_points() - 1),
...
{
    master = mesh_.elementisec->component_ele_idx()
...
unsigned int IntersectionQuadrature::master_dim()
    {return master->dim();}



unsigned int IntersectionQuadrature::slave_dim()
    {return slave->dim();}



void IntersectionQuadrature::intersection_point_to_vectors(const IntersectionPoint *point, arma::vec &vec1, arma::vec &vec2)
{
...
arma::vec IntersectionQuadrature::map_to_master(const arma::vec &point) const
{
...
arma::vec IntersectionQuadrature::map_to_slave(const arma::vec &point) const
{
	ASSERT_EQ_DBG( point.n_elem, dim ).error("Map to slave: point.n_elem != dim \n");
...
double IntersectionQuadrature::intersection_true_size() const {
