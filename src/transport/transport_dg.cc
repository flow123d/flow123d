...
void TransportDG<Model>::calculate_concentration_matrix()
{
    ...
    {
        typename DOFHandlerBase::CellIterator elem = Model::mesh_->bulk_begin() + feo->dh()->el_index(i_cell);
...
void TransportDG<Model>::assemble_mass_matrix()
{
    ...
    for (unsigned int i_cell=0; i_cell<Model::mesh_->get_el_ds()->lsize(); i_cell++)
    {
        typename DOFHandlerBase::CellIterator cell = Model::mesh_->bulk_begin() + feo->dh()->el_index(i_cell);
...
void TransportDG<Model>::assemble_fluxes_element_element()
{
    ...
    for (unsigned int iedg=0; iedg<feo->dh()->n_loc_edges(); iedg++)
    {
        ...
        for (int sid=0; sid<edg->n_sides; sid++)
        {
            ...
            for (unsigned int sbi=0; sbi<Model::n_substances(); sbi++)
                dg_penalty[sid][sbi] = data_.dg_penalty[sbi].value(cell->centre(), ele_acc);
        }
        ...
        for (unsigned int sbi=0; sbi<Model::n_substances(); sbi++)
        {
            ...
            for (int s1=0; s1<edg->n_sides; s1++)
            {
                for (int s2=s1+1; s2<edg->n_sides; s2++)
                {
                    ...
                    if (!feo->dh()->el_is_local(Model::mesh_->elem_index( edg->side(s1)->element()->id() ))
                            && !feo->dh()->el_is_local(Model::mesh_->elem_index( edg->side(s2)->element()->id() ))) continue;
...
#define JUMP(i,k,side_id)     ((side_id==0?1:-1)*fe_values[sd[side_id]]->shape_value(i,k))
                    ...
                    for (int n=0; n<2; n++)
                    {
                        if (!feo->dh()->el_is_local(Model::mesh_->elem_index( edg->side(sd[n])->element()->id() ))) continue;
...
void TransportDG<Model>::assemble_fluxes_boundary()
{
    ...
    for (unsigned int iedg=0; iedg<feo->dh()->n_loc_edges(); iedg++)
    {
        ...
        for (unsigned int sbi=0; sbi<Model::n_substances(); sbi++)
        {
            ...
            {
                // set up the parameters for DG method
                set_DG_parameters_boundary(side, qsize, dif_coef[sbi], transport_flux, fe_values_side.normal_vector(0), data_.dg_penalty[sbi].value(cell->centre(), ele_acc), gamma_l);
...
void TransportDG<Model>::assemble_fluxes_element_side()
{
    ...
    for (unsigned int inb=0; inb<feo->dh()->n_loc_nb(); inb++)
    {
        ...
        typename DOFHandlerBase::CellIterator cell_sub = Model::mesh_->bulk_begin() + Model::mesh_->elem_index( nb->element()->id() );
        ...
        int element_id[2];
        element_id[0] = Model::mesh_->elem_index( cell_sub->id() );
        element_id[1] = Model::mesh_->elem_index( cell->id() );
        ...
        for (unsigned int sbi=0; sbi<Model::n_substances(); sbi++) // Optimize: SWAP LOOPS
        {
            ...
            data_.fracture_sigma[sbi].value_list(fe_values_vb.point_list(), cell_sub->element_accessor(), frac_sigma);
...
void TransportDG<Model>::set_boundary_conditions()
{
    ...
    for (unsigned int loc_el = 0; loc_el < Model::mesh_->get_el_ds()->lsize(); loc_el++)
    {
        ...
        FOR_ELEMENT_SIDES(elm,si)
        {
            ...
            SideIter side = edg->side(0);
            typename DOFHandlerBase::CellIterator cell = Model::mesh_->bulk_begin() + Model::mesh_->elem_index( side->element()->id() );
...
double elem_anisotropy(const ElementIterator &e)
{
