          ...
          allocation_done(false)
{
    ...
    ret_coef.resize(n_subst_);
    sorption_sources.resize(n_subst_);
    ...
    for (unsigned int sbi = 0; sbi < n_subst_; sbi++) {
    	...
    	( (LinSys_PETSC *)ls_dt[sbi] )->set_from_input( in_rec.val<Input::Record>("solver") );
    }
    stiffness_matrix = new Mat[n_subst_];
    mass_matrix = new Mat[n_subst_];
...
void TransportDG<Model>::zero_time_step()
{
    ...
    if (balance_ != nullptr)
    {
		...
		for (unsigned int sbi=0; sbi<n_subst_; ++sbi)
		{
			...
			double mass = 0;
			balance_->calculate_mass(subst_idx[sbi], ls[sbi]->get_solution(), masses);
...
void TransportDG<Model>::update_solution()
{
    ...
    for (unsigned int i=0; i<n_subst_; i++)
    {
		...
		MatAXPY(m, 1./time_->dt(), mass_matrix[i], SUBSET_NONZERO_PATTERN);
    ...
    if (balance_ != nullptr)
    {
    	for (unsigned int sbi=0; sbi<n_subst_; ++sbi)
    	{
    		...
    		if (balance_->cumulative())
    		{
    			...
    			balance_->add_cumulative_source(subst_idx[sbi], mass-total_mass-sorption_sources[sbi]);
...
void TransportDG<Model>::assemble_fluxes_element_side()
{
	...
	vector<double> csection_lower(qsize), csection_higher(qsize), mm_coef_lower(qsize), mm_coef_higher(qsize), por_lower(qsize), por_higher(qsize);
		...
		for (unsigned int sbi=0; sbi<n_subst_; sbi++)
		{
			...
			for (unsigned int k=0; k<qsize; k++)
			{
				...
				double por_lower_over_higher = //por_lower[k]/por_higher[k];
					mm_coef_lower[k]*csection_higher[k]/(mm_coef_higher[k]*csection_lower[k]);
