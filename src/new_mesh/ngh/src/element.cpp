#include "new_mesh/ngh/include/element.h"
...
unsigned int TElement::GetNSides() const {
    return n_sides;
}
...
bool TElement::AreVVNeighbours(TProblem* problem, TElement* ele, IntersectionLocal* &insec) {
    TPoint P;
    //TPosition pos;
            ...
            switch (ele2->type) {
                case LINE:
                    if (problem->getCt_11() == ct_ratio1) {
                        ...
                        GetIntersection(*(ele1->abscissa), *(ele2->abscissa), insec);
                        if (insec != NULL) {
                            insec->set_elements(ele1, ele2);
                            return true;
                        }
                    }
                    break;
                case TRIANGLE:
                    if (problem->getCt_12() == ct_ratio1) {
                        ...
                        //cout << "E1_id, E2_id: " << ele1->id <<", " << ele2->id << endl;
                        GetIntersection(*(ele1->abscissa), *(ele2->triangle), insec);
                        if (insec != NULL) {
                            insec->set_elements(ele1, ele2);
                            return true;
                        }
