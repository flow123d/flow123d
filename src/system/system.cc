...
char *xstrcpy( const char *src )
{
	...
	ASSERT_PTR(src).error("NULL pointer as argument of function xstrcpy()\n");
	length = strlen( src ) + 1;
...
char *xstrtok( char *s1, const char *delim, int position )
{
	...
	ASSERT_PTR(delim).error("NULL pointer as delimiter in xstrtok()\n");
...
int xchomp( char * s )
{
    ...
    ASSERT(s).error("Can not chomp NULL string.");
...
int xmkdir( const char *s )
{
    ...
    ASSERT_PTR(s).error("NULL pointer as argument of function xmkdir()\n");
...
int xrmdir( const char *s )
{
    ...
    ASSERT_PTR(s).error("NULL pointer as argument of function xrmdir()\n");
    rc = rmdir( s );
...
int xchdir( const char *s )
{
    ...
    ASSERT_PTR(s).error("NULL pointer as argument of function xchdir()\n");
    rc = chdir( s );
...
int xremove( const char *fname )
{
    ...
    ASSERT_PTR(fname).error("NULL pointer as argument of function xremove()\n");
...
char *xgetcwd( void )
{
    ...
    ASSERT_PTR(rc(.error("Cannot get name of current working directory\n");
