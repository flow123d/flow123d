...
OutputTime::OutputDataPtr OutputTime::prepare_compute_data(std::string field_name,
    DiscreteSpace space_type, unsigned int n_rows, unsigned int n_cols, std::string fe_type)
{
    ...
    */
    ...
    if(current_step == 0 && it == od_vec.end() ) {
        ...
        od_vec.push_back( std::make_shared< ElementDataCache<T> >(field_name, n_rows*n_cols, size, fe_type) );
        it=--od_vec.end();
    }
    else{
        ASSERT(it != od_vec.end()).error("Try to add non-existing output field after first step.");
        *it = std::make_shared< ElementDataCache<T> >(field_name, n_rows*n_cols, size, fe_type);
    }
