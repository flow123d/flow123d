...
# ssh_pass=$4     # password ; just SSH key login allowed
...
ssh ${opt} ${ssh_remote} mkdir -p ${target_dir}

# copy the tar archive
scp -vr ${opt} package.tar ${ssh_remote}:${target_dir}/package.tar
...
# remove archive
ssh ${opt} ${ssh_remote} rm ${target_dir}/package.tar
