\section{1D column}\label{d-column}
...
no flow boundary condition on the edges (Figure \ref{fig:geom}).

\begin{figure}[htbp]
\centering
\includegraphics[width=1.00000\textwidth]{images/01_mesh_bc_flux.pdf}
\caption{a) the mesh; b) the boundary conditions; c) computed
piezometric head and flux.\label{fig:geom}}
\end{figure}
...
\subsubsection{Setting the computational
mesh}\label{setting-the-computational-mesh}
...
\subsubsection{Setting the model and physical
parameters}\label{setting-the-model-and-physical-parameters}
...
\subsubsection{Setting solver
parameters}\label{setting-solver-parameters}
...
\subsubsection{Setting output}\label{setting-output}
...
\subsection{Variant}\label{variant}
