...
\begin{equation}
    \label{eq:darcy}
    \vc v = -\tn K \grad H \quad\text{on }\Omega_d,\ \text{for $d=1,2,3$}.
\end{equation}
...
In \eqref{eq:darcy}, $\vc v_d$ \units{}{1}{-1} is \href{http://en.wikipedia.org/wiki/Superficial_velocity}{the superficial velocity},
...
\hyperA{DarcyFlowMH-Steady-BulkData::sigma}{$\sigma_d$} \units{}{3-d}{-1} are independent parameters in our setting however in practice
they should be related to the conductivity in direction (or plane) perpendicular to the fracture (channel). According to 
\cite{martin_modeling_2005} one can use
\[
  \sigma_3 = \frac{2\tn K_2 :\vc n_2\otimes\vc n_2 }{\delta_2}, \sigma_2^{i} = \frac{2\delta_2\tn K_1:{\vc n_1^i}\otimes{\vc n_1^i}}{\delta_1}
\]
