...
Its form is:
$$ F_S^i = \begin{cases} \varrho^i_S + (c_S^i-c^i)\sigma_S & \mbox{ if }c^i < c_S^i,\\ \varrho^i_S & \mbox{ else.}\end{cases} $$
Here $\varrho_S^i$ \units{1}{-d}{-1} is the \hyperA{TransportDG-BulkData::sources-density}{density of concentration sources}, $c_S^i$ is a \hyperA{TransportDG-BulkData::sources-conc}{concentration threshold} and $\sigma_S^i$ is the \hyperA{TransportDG-BulkData::sources-sigma}{concentration flux}.
