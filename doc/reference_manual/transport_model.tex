...
\begin{equation}
  ...
  q^c_{d+1,d} = \delta_{d+1}\sigma^c_{d+1,d} \th_d ( c_{d+1} - c_d) + \begin{cases}q^w_{d+1,d} c_{d+1} & \mbox{ if }q^w_{d+1,d}\ge 0,\\q^w_{d+1,d} \frac{\th_d}{\th_{d+1}} c_d & \mbox{ if }q^w_{d+1,d}<0,\end{cases}
\end{equation}
where
\begin{itemize}
\item $q^c_{d+1,d}$ \units{1}{-d}{-1} is the density of concentration flux from $\Omega_{d+1}$ to $\Omega_d$,
\item $\sigma^c_{d+1,d}$ \units{}{1}{-1} is a \hyperA{TransportDG-BulkData::sigma-c}{transition parameter}.
...
\[
  \sigma^c_{d+1,d} \approx\frac{\delta_{d+1}}{\delta_d}\D_d:\n\otimes\n.
\]
