...
\end{itemize}
		...
		\RecKey
			{Mesh::global-observe-search-radius}
			{global{\_}observe{\_}search{\_}radius}
			{{Double [0, +inf)}}{}
			{ \ValueDefault{0.001}}
			{{{Maximal distance of observe point from Mesh relative to its size (bounding box). Value is global and it can be rewrite at arbitrary ObservePoint by setting the key search{\_}radius.}% 
}}
\end{RecordType}
\begin{AbstractType}
	{IT::Region}
...
\begin{RecordType}
	...
	{{{R3{\_}to{\_}R[3,3] Field piecewise constant on mesh elements.}% 
		...
		\RecKey
			{FieldElementwise::mesh-data-file}
			{mesh{\_}data{\_}file}
			{{Filename}}{}
			{ \it{Obligatory}}
...
point{\_}data/node{\_}data, cell{\_}data/element{\_}data, -/element{\_}node{\_}data, native/-.}\\{
...
\begin{RecordType}
	...
	{{{R3{\_}to{\_}R Field piecewise constant on mesh elements.}% 
		...
		\RecKey
			{FieldElementwise-2::mesh-data-file}
			{mesh{\_}data{\_}file}
			{{Filename}}{}
			{ \it{Obligatory}}
			{{{Input file with ASCII GMSH file format.}% 
...
Without snapping we make a projection to the initial element.}% 
		...
		\RecKey
			...
			{{{Global value is define in Mesh by the key global{\_}observe{\_}search{\_}radius.}% 
}}
\end{RecordType}
\begin{RecordType}
	{IT::EquationOutput}
