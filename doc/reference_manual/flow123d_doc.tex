...
\vspace{3ex}
\noindent This work was supported by S\'URAO within project Decovalex 2015, SO2013-077.
\pagebreak
\noindent
...
%\tbd{TODO-JAN BŘEZINA: update after all new sections are written.}
...
explicit Euler method for time discretization and finite volume method for space discretization.
The second solute transport model describes general advection with hydrodynamic dispersion for several substances. 
It uses implicit Euler method for time discretization and discontinuous Galerkin method of
...
Download the Linux installation package archive \verb'Flow123d-2.1.0-linux.tar.gz' and extract it into any folder using
the command
\begin{verbatim}
  > tar -xzf Flow123d-2.1.0-linux.tar.gz 
\end{verbatim}
This will create a directory \verb'Flow123d-2.1.0-linux'. In the next step, navigate to the \verb'Flow123d-2.1.0-linux' directory
and execute the \verb'install.sh' script:
\begin{verbatim}
  > cd Flow123d-2.1.0-linux
  > ./install.sh
...
%\tbd{Is it standard part of which Windows or how it can be obtained.}

Make sure \verb'powershell' command is in the system PATH. \href{http://www.powershelladmin.com/wiki/PowerShell_Executables_File_System_Locations}{Powershell executable location}
...
To install Flow123d on Windows, first download the Windows installation package archive \verb'Flow123d-2.1.0-windows.zip' and extract it to any folder.
This will create a directory \verb'Flow123d-2.1.0-windows' navigate to that directory and execute \verb'install.bat' file.
