...
\section{Installing Flow123d (Linux)}
Download \verb'Flow123d-<version>-Linux.tar.gz' package archive and extract it to any folder. For extraction, use the command
\begin{verbatim}
  > tar -xzf Flow123d-<version>-Linux.tar.gz 
\end{verbatim}
This will create a directory \verb'Flow123d-<version>-Linux' which we further refer as \verb'<flow base>' directory.
The binary and some supporting scripts depends on working python interpreter.


\section{Running Flow123d (Linux)}
On the Linux system the program can be started either directly or through a script \verb'flow123d.sh', 
both placed in the \verb'bin' directory of the flow base directory. When started directly, e.g. by the command
\begin{verbatim}
  > flow123d example.yaml
\end{verbatim}
the program requires at least one argument, which is the name of the principal input file. Full list of possible command line arguments is as follows.
...
\tbd{TODO-JAN HYBŠ: modify flow123d.sh to be just abbreviation of python exec\_parallel.py flow123d. Update description.}
...
\section{Installing Flow123d (Windows)}
\tbd{TODO-JAN HYBŠ: install instructions}

\section{Running Flow123d (Windows)}
\tbd{TODO-JAN HYBŠ: update this section specific to running under docker. For parameters refer to the Linux doc.}
...
\tbd{TODO-JAN STEBEL: place selected tutorials here or on some other appropriate place}
\input{tutorial}
