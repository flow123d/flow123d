...
\hyperA{TransportOperatorSplitting::reaction-term}{\tt reaction\_term}, lines 61 -- 100. Go to section \ref{sec:reaction_term}
to see how the models can be chained.

The type of the first process is determined by {\tt TYPE="DualPorosity"}, on line 62. 
The \hyperA{DualPorosity::input-fields}{\tt input\_fields}
...
Next, we define the equilibrial sorption model such that {\tt SorptionMobile} type takes place in the mobile 
zone of the dual porosity model while {\tt SorptionImmobile} type takes place in its immobile zone, see lines 76 and 93.

On lines 77 -- 89, we set the sorption related input information. The solvent is water so the \hyperA{Sorption::solvent-density}{\tt solvent\_density} 
is supposed to be constant all over the simulated area. The vector \hyperA{Sorption::substances}{\tt substances} 
...
\hyperA{Sorption::solubility}{\tt solubility} define the upper bound of aqueous concentration which can appear.
...
The record \hyperA{Sorption::input-fields}{\tt input\_fields} covers the region specific parameters.
All implemented types of sorption can take the rock density in the region into account. The value of 
\hyperA{Sorption-Data::rock-density}{\tt rock\_density} is a constant in our case. 
The \hyperA{Sorption-Data::sorption-type}{\tt sorption\_type} reperesents the empirically determined isotherm 
...
For further details, see mathematical description in Section \ref{sec:sorp_math}. Isothermally described sorption simulation can be used in 
