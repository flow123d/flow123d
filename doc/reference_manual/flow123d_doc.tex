...
On lines 26 -- 33 we specify which output fields should be written to the output stream (that means particular output file, with given format).
...
On line 44, we set names of transported substances, here it is the age of the water and the uranium 235. On lines 46 -- 55, we set the input fields, in particular the \hyperA{TransportOperatorSplitting::porosity}{\tt porosity}, sources of concentration and the initial concentrations 
(one for every substance). However, on line 48 we see only single value since an automatic conversion is applied to turn the scalar zero into the zero vector (of size 2). 
On lines 52 -- 54, we set the boundary fields, namely the concentration on the inflow part of the boundary.
We need not to specify the type of the condition since currently there is only one type for this transport model.
On lines 57 -- 98, we specify the dual porosity and adsorption mechanisms (see comments below).
On line 95, notice the reference pointing to the definition of input fileds on lines 78 -- 86.
On lines 100 -- 105 we specify which output fields should be written to the output stream. Currently, we support output into VTK and GMSH data format.
In the output record for time-dependent process we have to specify the {\tt time\_step} (line 103) which determines the frequency of saving.
