...
\usepackage{caption}
\usepackage{packages/subcaption}
...
basic notation and assumptions about computational domains and meshes that combines different dimensions. In the next section we
derive approximation of thin fractures by lower dimensional interfaces for general transport process. Latter sections describes details for models of particular
physical processes. 
...
In this section, we shall derive a model for a general advection-diffusion process on a domain of mixed dimensions using
simple approximations inspired by the paper \cite{martin_modeling_2005} due to Martin et al. Let us consider a fracture as a strip domain 
\[
...
We consider the normal vector  $\vc n=(1,0,0)$.
An advection-diffusion process is given by equations:
\begin{align}
  \label{eq:fr:continuity}
  \prtl_t c_i + \div \vc q_i &= f_i&&  \text{on } \Omega_i,\ i=1,2,f,\\
  \label{eq:fr:flux}
  \vc q_i &= - \tn K_i\grad u_i + \vc b_i c_i&& \text{on } \Omega_i,\ i=1,2,f,\\
  \label{eq:fr:Dirichlet}
  u_i &= u_f&& \text{on } \gamma_i,\ i=1,2,\\
  \label{eq:fr:Neumann}
  \vc q_i \cdot \vc n &= \vc q_f \cdot \vc n&& \text{on } \gamma_i,\ i=1,2
\end{align}
where $c_i=c_i(u_i)$ is the conservative quantity and $u_i$ is the principal unknown , $q_i$ is the flux of $c_i$, $f_i$ is the source term,
$\tn K_i$ is the diffusivity tensor and $\vc b_i$ is the velocity field. We assume that the tensor $\tn K_f$ is symmetric positive definite 
with one eigenvector in the direction $\vc n$. Consequently the tensor has a form:
\[
 K_f = \begin{pmatrix} 
            k_n & 0  \\
            0 & \tn K_t
       \end{pmatrix}
\]
Further more, we assume that $\tn K_f(x, \vc y)=\tn K_f(\vc y)$ is constant in the normal direction.

Our next aim is to integrate equations on the fracture $\Omega_f$ in the normal direction 
and obtain their approximations on the surface $\gamma=\Omega_f \cap \{x=0.5\}$ running through the middle of the fracture. 
For the sake of clarity, we will not write subscript $f$ for quantities on the fracture. 
To make following procedure mathematicaly correct we have to assume that functions
$\prtl_x c$, $\prtl_x \grad_{\vc y} u$, $\prtl_x \vc b_{\vc y}$ are continuous and bounded on $\Omega_f$. Here and later on 
$\vc b_x=(\vc b \cdot \vc n)\, \vc n$ is the normal part of the velocity field and $\vc b_{\vc y} = \vc b - \vc b_x$ is the tangential part.
Same notation will be used for normal and tangential part of the field $\vc q$.
...
\begin{equation}
   ...
   \prtl_t (\delta C) + \vc q_2 \cdot \vc n - \vc q_1 \cdot \vc n + \div \vc Q = F,
\end{equation}
where for the first term, we have used mean value theorem, first order Taylor expansion, 
and boundedness of $\prtl_x c$ to obtain approximation:
\[
    \int_0^\delta c(x,\vc y) \d x=\delta c(\xi_{\vc y}, \vc y) = \delta C(\vc y) + O(\delta^2\abs{\prtl_x c}),
\]
where
\[
    C(\vc y)=c(\delta / 2,\vc y)=c(u(\delta/2,\vc y))=c(U(\vc y)).
\]
Next two terms in \eqref{eq:fracture_continuity} comes from the exact integration 
of the divergence of the normal flux $\vc q_x$.
Integration of the divergence of the tangential flux $\vc q_{\vc y}$ gives the fourth term, where we introduced
\[
\vc Q(\vc y) = \int_0^\delta \vc q_{\vc y}(x, \vc y) \d x.
\]
In fact, this flux on $\gamma$ is scalar for the case $d=2$. Finally, we integrate the right-hand side to get 
\[
    F(\vc y)=\int_0^\delta f(x,\vc y) \d x. 
\]


Due to the particular form of the tensor $\tn K_f$, we can separately integrate tangential and normal
...
and
\[
 \int_0^\delta \big(\vc b_{\vc y} c\big)(x, \vc y) \d x 
  = \delta \vc B(\vc y) C(\vc y) + O\big(\delta^2 \abs{\prtl_x(\vc b_{\vc y} c)} \big)
\]
where
\[
...
\begin{equation}
   ...
   \vc Q = -\tn K_t \delta \grad_{\vc y} U + \delta \vc B C.
\end{equation}


So far, we have derived equations for the state quantities $U$ and $\vc Q$ on the fracture maniflod $\gamma$. In order to
get well possed problem, we have to prescribe two conditions for boundaries $\gamma_i$, $i=1,2$. To this end, we
perform integration of the normal flux $\vc q_x$, given by \eqref{eq:fr:flux}, separately for the left and right half of the fracture.
Similarly as before we use approximations
\[
 \int_0^{\delta/2} \vc q_x \d x = (\vc q_1 \cdot \vc n_1)\frac{\delta}{2} + O(\delta^2 \abs{\prtl_x \vc q_x})
\]
and 
\[
 \int_0^{\delta/2} \vc b_x c \d x = (\vc b_1 \cdot \vc n_1)\tilde{c}_1\frac{\delta}{2} + O(\delta^2 \abs{\prtl_x \vc b_x}\abs{c} + \delta^2\abs{\vc b_x}\abs{\prtl_x c})
\]
and their counter parts on the interval $(\delta/2, \delta)$ to get
\begin{align}
    \label{eq:fracture_normal_1}
     \vc q_1 \cdot \vc n_1 &= -\frac{2k_n}{\delta} (C - c_1) + \vc b_1\cdot \vc n_1 \tilde{c}_1\\
    \label{eq:fracture_normal_2}
    \vc q_2 \cdot \vc n_2 &= -\frac{2k_n}{\delta} (C - c_2) + \vc b_2\cdot \vc n_2 \tilde{c}_2
\end{align}
where $\tilde c_i$ can be any convex combination of $c_i$ and $C$. Equations \eqref{eq:fracture_normal_1}  
and \eqref{eq:fracture_normal_2} have meaning of a semi-discretized flux from domains $\Omega_i$ into fracture.
In order to get stable numerical scheme, we introduce kind of upwind already at this level using different convex 
combination for different flow direction:
\begin{align}
   \notag 
   \vc q_i \cdot \vc n_i
       = &-\sigma_i (C - c_i)      \\ 
   \notag
      &+ \big[\vc b_i\cdot \vc n_i\big]^{+} \big(\xi c_i + (1-\xi) C\big)       \\
      \label{eq:fracture_normal}
      &+ \big[\vc b_i\cdot \vc n_i\big]^{-} \big((1-\xi) c_i + \xi C\big), \qquad i=1,2
\end{align}
where $\sigma_i = \frac{2k_n}{\delta}$ is the transition coefficient and parameter $\xi\in [\frac12, 1]$ can be used to interpolate
...
\eqref{eq:fracture_normal} describe general form of the advection-diffusion process on the fracture and its communication with 
the surrounding continuum which later apply to individual processes.
