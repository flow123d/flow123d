...
\[ \left(\varrho_l c_l T \vc q - \delta\Lambda\nabla T\right)\cdot\vc n = \delta(f^T_N+\sigma^T_R(T-T_R)) + \varrho_l c_l T_A\vc q\cdot\vc n. \]
with user-defined \hyperA{HeatTransfer-DG-Data::bc-flux}{energy flux} $f^T_N$ [J$\mathrm{m}^{-2}\mathrm{s}^{-1}$],
\hyperA{HeatTransfer-DG-Data::bc-robin-sigma}{transition parameter} $\sigma^T_R$ [J$\mathrm{m}^{-2}\mathrm{s}^{-1}\mathrm{K}^{-1}$], \hyperA{HeatTransfer-DG-Data::bc-temperature}{reference temperature} $T_R$ and \hyperA{HeatTransfer-DG-Data::bc-ad-temperature}{advected temperature} $T_A$.
\item \textbf{diffusive\_flux} Finally on $\Gamma_{DF}$ we prescribe diffusive energy flux through \hyperA{HeatTransfer-DG-Data::bc-flux}{given energy flux} $f^T_N$, \hyperA{HeatTransfer-DG-Data::bc-robin-sigma}{transition parameter} $\sigma^T_R$ and \hyperA{HeatTransfer-DG-Data::bc-temperature}{reference temperature} $T_R$:
\[ -\delta\Lambda\nabla T\cdot\vc n = \delta(f^T_N+\sigma^T_R(T-T_R)). \]
\end{itemize}
