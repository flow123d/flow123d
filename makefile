...
TUTOR_DIR=$(SOURCE_DIR)/tests/05_tutorial
...
html-doc: 
	...
	python3 $(SOURCE_DIR)/src/py123d/bin/ist_script.py --input=$(DOC_DIR)/input_reference.json --output=$(BUILD_DIR)/htmldoc/html/src/index.html --format=html
...
# Create user manual using LaTeX sources and generated input reference
# use makefile generated by cmake, but independent on 'make cmake'
# It tries to update tutorials and input reference file.
.PHONY: ref-doc
ref-doc: update-input-ref update-tutorials fast-ref-doc


.PHONY: update-input-ref
# update generated reference doc for input file (if flow123d exists)
# 
update-input-ref:
	@if [ -x $(BUILD_DIR)/bin/flow123d ]; \
	then \
		$(BUILD_DIR)/bin/flow123d --input_format "$(DOC_DIR)/input_reference.json"; \
		python3 $(SOURCE_DIR)/src/py123d/bin/ist_script.py --input=$(DOC_DIR)/input_reference.json --output=$(DOC_DIR)/input_reference.tex --format=tex --debug --log info; \
	else \
